...
export function AISourcingCopilot() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0E0E10] text-slate-900 dark:text-[#E4E4E7] p-4 sm:p-6 lg:p-8">
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-[#27272A]">
        <div className="flex items-center gap-3.5">
          <div className="p-3 rounded-2xl bg-gradient-to-tr from-purple-600/20 to-blue-500/20 border border-purple-500/30 text-purple-600 dark:text-purple-400">
        ...
        {/* Tab Switcher */}
        <div className="flex items-center bg-slate-200/70 dark:bg-[#141416] p-1 rounded-xl border border-slate-300/80 dark:border-[#27272A]">
          <button
      ...
      {/* Success Banner */}
      {convertedSuccess && (
        <div className="mt-6 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-sm font-semibold text-emerald-800 dark:text-emerald-300">
      ...
      {activeTab === "smart-rfq" && (
        <div className="mt-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Smart RFQ Lots & Pricing Anomaly Radar</h2>
          ...
          <div className="space-y-4">
            {loadingDrafts ? (
              <div className="space-y-3">
                {[1, 2].map((idx) => (
                  <div key={idx} className="h-44 rounded-2xl bg-slate-100 dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] animate-pulse" />
                ))}
              </div>
            ) : drafts.length > 0 ? (
              drafts.map((d) => (
                <div
                  key={d.id}
                  className="p-5 rounded-2xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] space-y-4 hover:border-slate-300 dark:hover:border-[#38383E] shadow-sm transition-all"
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-[#27272A]">
                    <div>
                      <div className="flex items-center gap-2.5">
                        <h3 className="text-base font-bold text-slate-900 dark:text-white">{d.rfq_title}</h3>
                        <span
                  ...
                  {d.anomaly_flags && d.anomaly_flags.length > 0 && (
                    <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 space-y-1.5">
                      ...
                      <div className="space-y-1">
                        {d.anomaly_flags.map((af, idx) => (
                          <div key={idx} className="flex items-center justify-between text-xs text-slate-800 dark:text-[#E4E4E7]">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs">
                      <thead>
                        <tr className="border-b border-slate-200 dark:border-[#222226] text-slate-500 dark:text-[#71717A]">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-[#1F1F23]">
                        {d.lots.map((lot, idx) => (
                          <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-[#18181B] transition-colors">
                            <td className="py-2.5 font-mono text-slate-500 dark:text-[#A1A1AA]">{lot.lot_number}</td>
                            <td className="py-2.5 font-medium text-slate-900 dark:text-white">{lot.lot_name}</td>
                            <td className="py-2.5 text-slate-600 dark:text-[#A1A1AA]">{lot.quantity}</td>
                            <td className="py-2.5 font-semibold text-slate-900 dark:text-white">
                              ₹{formatINR(lot.estimated_unit_price)}
                            </td>
                            <td className="py-2.5 text-slate-600 dark:text-[#A1A1AA]">
              ...
              <div className="p-12 text-center rounded-2xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] shadow-sm">
                <Layers className="w-10 h-10 text-slate-400 dark:text-[#71717A] mx-auto mb-3" />
                <p className="text-sm font-semibold text-slate-900 dark:text-white">No Smart RFQs generated yet</p>
                <p className="text-xs text-slate-500 dark:text-[#71717A] mt-1">Click Generate Smart RFQ to cluster PR items autonomously</p>
              </div>
            )}
          </div>
        </div>
      ...
      {activeTab === "negotiation-bot" && (
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Active Bot Sessions</h2>
              <button
                onClick={() => setStartSessionModalOpen(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold transition-all shadow-xs cursor-pointer"
              >
            ...
            <div className="space-y-2.5">
              {loadingSessions ? (
                <div className="h-32 rounded-xl bg-slate-100 dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] animate-pulse" />
              ) : sessions.length > 0 ? (
                sessions.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => setSelectedSessionId(s.id)}
                    className={`w-full text-left p-4 rounded-xl border transition-all flex flex-col justify-between gap-2 cursor-pointer ${
                      selectedSessionId === s.id
                        ? "bg-purple-50/80 dark:bg-[#1C1C20] border-purple-500 shadow-md ring-1 ring-purple-500/20"
                        : "bg-white dark:bg-[#141416] border-slate-200 dark:border-[#27272A] hover:border-slate-300 dark:hover:border-[#38383E] shadow-2xs"
                    }`}
                  >
                    ...
                    <div className="text-[11px] text-slate-500 dark:text-[#71717A] flex items-center justify-between w-full">
                      <span>{s.vendor_name}</span>
                      <span>Round {s.current_round} / {s.max_rounds}</span>
                    </div>

                    <div className="pt-2 border-t border-slate-100 dark:border-[#222226] flex items-center justify-between w-full text-xs">
                      <span className="text-slate-600 dark:text-[#A1A1AA]">Current: ₹{formatINR(s.current_bid_price)}</span>
                ...
                <div className="p-8 text-center rounded-xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] text-xs text-slate-500 dark:text-[#71717A] shadow-xs">
          ...
          <div className="lg:col-span-2 space-y-4">
            {selectedSession ? (
              <div className="p-6 rounded-2xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] space-y-6 shadow-sm">
                {/* Session Header */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-[#27272A]">
                  <div>
                    <h3 className="text-base font-bold text-slate-900 dark:text-white">{selectedSession.item_description}</h3>
                    <p className="text-xs text-slate-500 dark:text-[#A1A1AA] mt-0.5">
                      Partner: <span className="text-slate-900 dark:text-white font-medium">{selectedSession.vendor_name}</span> • Strategy:{" "}
                      <span className="font-semibold text-purple-600 dark:text-purple-400">{selectedSession.concession_strategy}</span>
                    </p>
                  </div>

                  <div className="flex items-center gap-4 text-xs">
                    <div className="text-right">
                      <span className="text-slate-500 dark:text-[#71717A] block">Target Price</span>
                      <span className="font-bold text-emerald-600 dark:text-emerald-400">
                        ₹{formatINR(selectedSession.target_price)}
                      </span>
                    </div>
                    <div className="text-right">
                      <span className="text-slate-500 dark:text-[#71717A] block">Max Ceiling</span>
                    ...
                    <div
                      ...
                      className={`p-4 rounded-xl border ${
                        r.bidder_type === "AI_BOT"
                          ...
                          : "bg-slate-50 dark:bg-[#1B1B1E] border-slate-200 dark:border-[#2E2E34]"
                      }`}
                    >
                      ...
                      <p className="text-xs text-slate-600 dark:text-[#A1A1AA] leading-relaxed">{r.rationale}</p>
                    </div>
                  ))}
                </div>
                ...
                {selectedSession.bot_status === "ACTIVE" ? (
                  <div className="pt-4 border-t border-slate-100 dark:border-[#27272A] space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-[#A1A1AA] flex items-center gap-1.5">
                      <Scale className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" />
                      Simulate Supplier Counter-Bid
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        <label className="block text-[11px] text-slate-500 dark:text-[#71717A] mb-1">Proposed Counter Price (₹)</label>
                        <input
                          type="number"
                          value={supplierCounterInput}
                          onChange={(e) => setSupplierCounterInput(Number(e.target.value))}
                          className="w-full px-3 py-2 rounded-lg bg-slate-50 dark:bg-[#1F1F23] border border-slate-200 dark:border-[#333338] text-xs text-slate-900 dark:text-white focus:outline-none focus:border-purple-500"
                        />
                      </div>
                      <div className="md:col-span-2">
                        <label className="block text-[11px] text-slate-500 dark:text-[#71717A] mb-1">Supplier Concession Message</label>
                        <div className="flex gap-2">
                          <input
                            type="text"
                            value={supplierMessageInput}
                            onChange={(e) => setSupplierMessageInput(e.target.value)}
                            className="flex-1 px-3 py-2 rounded-lg bg-slate-50 dark:bg-[#1F1F23] border border-slate-200 dark:border-[#333338] text-xs text-slate-900 dark:text-white focus:outline-none focus:border-purple-500"
                          />
                          <button
                            onClick={handleSubmitCounter}
                            disabled={submitCounterMutation.isPending}
                            className="px-4 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold transition-all flex items-center gap-1.5 disabled:opacity-50 cursor-pointer shadow-xs"
                          >
              ...
              <div className="p-16 text-center rounded-2xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] text-xs text-slate-500 dark:text-[#71717A] shadow-xs">
      ...
      {activeTab === "supplier-radar" && (
        <div className="mt-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Multi-Factor Supplier Recommendation Radar</h2>
              <p className="text-xs text-slate-500 dark:text-[#A1A1AA]">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white dark:bg-[#1C1C1F] hover:bg-slate-50 dark:hover:bg-[#252529] border border-slate-200 dark:border-[#2E2E32] text-xs font-semibold text-slate-700 dark:text-white transition-all shadow-xs cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {loadingRadar ? (
              [1, 2, 3].map((idx) => (
                <div key={idx} className="h-64 rounded-2xl bg-slate-100 dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] animate-pulse" />
              ))
            ) : radarScores.length > 0 ? (
              radarScores.map((score) => (
                <div
                  key={score.id}
                  className="p-5 rounded-2xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] hover:border-slate-300 dark:hover:border-[#38383E] transition-all space-y-4 shadow-sm"
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-sm font-bold text-slate-900 dark:text-white">{score.vendor_name}</h3>
                      <p className="text-xs text-slate-500 dark:text-[#71717A]">{score.category_name}</p>
                    </div>
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                  ...
                  {/* Overall Fit Metric */}
                  <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#19191D] border border-slate-200 dark:border-[#2C2C32] flex items-center justify-between">
                    <span className="text-xs text-slate-600 dark:text-[#A1A1AA] font-medium">Overall AI Suitability</span>
                    <span className="text-lg font-bold text-slate-900 dark:text-white">{score.overall_fit_score} / 100</span>
                  </div>

                  {/* Score Breakdown Bars */}
                  <div className="space-y-2 text-xs">
                    <div>
                      <div className="flex justify-between text-[11px] text-slate-500 dark:text-[#A1A1AA] mb-1">
                        <span>Quality (35%)</span>
                        <span className="font-semibold text-slate-900 dark:text-white">{score.quality_score}%</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-slate-100 dark:bg-[#27272A] overflow-hidden">
                        <div
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] text-slate-500 dark:text-[#A1A1AA] mb-1">
                        ...
                        <span className="font-semibold text-slate-900 dark:text-white">{score.price_competitiveness_score}%</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-slate-100 dark:bg-[#27272A] overflow-hidden">
                        <div
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] text-slate-500 dark:text-[#A1A1AA] mb-1">
                        ...
                        <span className="font-semibold text-slate-900 dark:text-white">{score.lead_time_score}%</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-slate-100 dark:bg-[#27272A] overflow-hidden">
                        <div
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] text-slate-500 dark:text-[#A1A1AA] mb-1">
                        ...
                        <span className="font-semibold text-slate-900 dark:text-white">{score.esg_score}%</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-slate-100 dark:bg-[#27272A] overflow-hidden">
                        <div
                  ...
                  {score.insights?.strengths && score.insights.strengths.length > 0 && (
                    <div className="pt-2 border-t border-slate-100 dark:border-[#222226]">
                      <span className="text-[10px] uppercase font-semibold text-slate-500 dark:text-[#71717A] block mb-1">
                        Verified Strengths:
                      </span>
                      <div className="flex flex-wrap gap-1">
                        {score.insights.strengths.map((str, idx) => (
                          <span
                            key={idx}
                            className="text-[10px] px-2 py-0.5 rounded bg-slate-100 dark:bg-[#1F1F23] text-slate-700 dark:text-[#A1A1AA]"
                          >
                            {str}
                          </span>
                        ))}
              ...
              <div className="col-span-3 p-12 text-center rounded-2xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#27272A] shadow-sm">
                <Radar className="w-10 h-10 text-slate-400 dark:text-[#71717A] mx-auto mb-3" />
                <p className="text-sm font-semibold text-slate-900 dark:text-white">No radar assessments generated yet</p>
                <p className="text-xs text-slate-500 dark:text-[#71717A] mt-1">Click Refresh Radar above to compute multi-factor scores</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overscroll-contain">
          <div className="w-full max-w-md rounded-2xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#2E2E32] p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-[#27272A]">
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Generate Smart RFQ Draft</h3>
              <button onClick={() => setCreateRfqModalOpen(false)} className="text-slate-400 hover:text-slate-700 dark:text-[#71717A] dark:hover:text-white cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">RFQ Package Title</label>
                <input
                  type="text"
                  value={rfqTitleInput}
                  onChange={(e) => setRfqTitleInput(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg bg-slate-50 dark:bg-[#1F1F23] border border-slate-200 dark:border-[#333338] text-slate-900 dark:text-white focus:outline-none focus:border-purple-500"
                />
              </div>
              <div>
                <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Primary Lot Description</label>
                <input
                  type="text"
                  value={lotItemDesc}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Quantity</label>
                  <input
                    type="number"
                    value={lotQty}
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Quoted Unit Price (₹)</label>
                  <input
                    type="number"
                    value={lotPrice}
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-[#27272A]">
              <button
                ...
                className="px-4 py-2 rounded-lg text-xs font-medium text-slate-600 dark:text-[#A1A1AA] hover:text-slate-900 dark:hover:text-white cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="px-5 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold shadow-xs cursor-pointer disabled:opacity-50"
              >
      ...
      {startSessionModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overscroll-contain">
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-[#141416] border border-slate-200 dark:border-[#2E2E32] p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-[#27272A]">
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Start Autonomous Negotiation Session</h3>
              <button onClick={() => setStartSessionModalOpen(false)} className="text-slate-400 hover:text-slate-700 dark:text-[#71717A] dark:hover:text-white cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Target Vendor *</label>
                <select
              ...
              <div>
                <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Item / Scope Description</label>
                <input
                  type="text"
                  value={botItemDesc}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Opening Quote (₹)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Target Price (₹)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Max Ceiling (₹)</label>
                  <input
                    type="number"
                    value={botMaxPrice}
              ...
              <div>
                <label className="block font-semibold text-slate-700 dark:text-[#A1A1AA] mb-1">Concession Strategy</label>
                <select
                  value={botStrategy}
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-[#27272A]">
              ...
              <button
                ...
                className="px-5 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold disabled:opacity-50 cursor-pointer shadow-xs"
              >
