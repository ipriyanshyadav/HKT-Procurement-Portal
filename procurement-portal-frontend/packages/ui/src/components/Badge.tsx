"use client";

import React, { ReactNode } from 'react';

export type BadgeVariant =
  | 'approved'
  | 'pending'
  | 'rejected'
  | 'draft'
  | 'review'
  | 'active'
  | 'completed'
  | 'in_approval'
  | 'cancelled'
  | 'submitted'
  | 'warning'
  | 'info';

export interface BadgeProps {
  variant?: BadgeVariant | string;
  children: ReactNode;
  className?: string;
  dot?: boolean;
}

export function Badge({
  variant = 'draft',
  children,
  className = '',
  dot = true,
}: BadgeProps) {
  const normVariant = variant.toLowerCase().replace(/[\s-]/g, '_');

  let variantClass = 'badge-draft';
  if (['approved', 'active', 'completed', 'delivered', 'paid', 'verified', 'clean', 'ready', 'resolved', 'qualified', 'success', 'converted'].includes(normVariant)) {
    variantClass = 'badge-approved';
  } else if (['pending', 'in_approval', 'under_review', 'submitted', 'warning', 'scanning', 'pending_activation', 'pending_erp', 'resubmission_requested', 'bid_submission'].includes(normVariant)) {
    variantClass = 'badge-pending';
  } else if (['rejected', 'cancelled', 'overdue', 'failed', 'inactive', 'suspended', 'compliance_hold', 'blacklisted', 'infected', 'expired', 'danger', 'destructive'].includes(normVariant)) {
    variantClass = 'badge-rejected';
  } else if (['review', 'rfq_issued', 'info', 'open', 'bid_open', 'invited', 'registration_in_progress', 'primary', 'in_sourcing'].includes(normVariant)) {
    variantClass = 'badge-review';
  }

  return (
    <span
      className={`apple-badge ${variantClass} ${!dot ? 'before:hidden' : ''} ${className}`}
    >
      {children}
    </span>
  );
}
