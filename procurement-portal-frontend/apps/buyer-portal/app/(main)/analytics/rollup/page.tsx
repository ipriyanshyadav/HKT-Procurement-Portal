...
export default function CrossTenantRollupPage() {
    ...
    return (
      <div className="p-8 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8 animate-pulse">
          <div>
            <div className="h-7 w-72 bg-slate-200 dark:bg-neutral-800 rounded mb-2" />
            <div className="h-4 w-96 bg-slate-100 dark:bg-neutral-850 rounded" />
          </div>
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-[#2e2e32] pb-6">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-400 mb-1">
            <Link href="/analytics" className="inline-flex items-center gap-1 hover:underline text-slate-500 dark:text-neutral-400">
              <ArrowLeft className="w-3.5 h-3.5" /> Analytics
            </Link>
            <span>/</span>
            <span className="flex items-center gap-1">
              <Globe className="w-3.5 h-3.5" /> Multi-Tenant Enterprise Intelligence
            </span>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            onClick={() => refetch()}
            disabled={isFetching}
            className="flex items-center gap-2 px-3 py-2 text-xs font-medium rounded-lg bg-white dark:bg-[#1C1C1F] hover:bg-slate-50 dark:hover:bg-[#252529] border border-slate-200 dark:border-[#2e2e32] text-slate-700 dark:text-neutral-300 transition-colors shadow-2xs"
          >
          ...
          <Link
            href="/analytics"
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg bg-blue-600 hover:bg-blue-500 text-white transition-colors shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-xl bg-white dark:bg-[#1C1C1F] border border-slate-200 dark:border-[#2e2e32] shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500 dark:text-neutral-400">Total Group Spend</span>
        ...
        <div className="p-5 rounded-xl bg-white dark:bg-[#1C1C1F] border border-slate-200 dark:border-[#2e2e32] shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500 dark:text-neutral-400">Group PO Volume</span>
        ...
        <div className="p-5 rounded-xl bg-white dark:bg-[#1C1C1F] border border-slate-200 dark:border-[#2e2e32] shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500 dark:text-neutral-400">Active Vendor Base</span>
        ...
        <div className="p-5 rounded-xl bg-white dark:bg-[#1C1C1F] border border-slate-200 dark:border-[#2e2e32] shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500 dark:text-neutral-400">Avg PR-to-PO Cycle</span>
      ...
      <div className="bg-white dark:bg-[#1C1C1F] border border-slate-200 dark:border-[#2e2e32] rounded-xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-slate-100 dark:border-[#2e2e32] flex items-center justify-between">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 dark:bg-[#252529] text-slate-600 dark:text-neutral-400 uppercase tracking-wider font-semibold border-b border-slate-200 dark:border-[#2e2e32]">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-[#2e2e32]">
              {(rollup?.entities ?? []).map((entity) => (
                <tr key={entity.entity_id} className="hover:bg-slate-50/80 dark:hover:bg-[#252529]/60 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-slate-900 dark:text-white">{entity.entity_name}</div>
                    <div className="text-[11px] font-mono text-slate-500 dark:text-neutral-500">{entity.entity_id.slice(0, 8)}...</div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-neutral-800 text-slate-700 dark:text-neutral-300 font-mono text-[11px] border border-slate-200 dark:border-neutral-700">
                      {entity.country_code}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-mono text-slate-700 dark:text-neutral-200">
                    {entity.po_count}
                  </td>
                  <td className="px-6 py-4 font-mono text-slate-700 dark:text-neutral-300">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button
                      ...
                      className="px-2.5 py-1 text-xs font-medium rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-neutral-800 dark:hover:bg-neutral-700 text-slate-700 dark:text-neutral-200 border border-slate-300 dark:border-neutral-700 transition-colors disabled:opacity-50"
                    >
                      Switch
                    </button>
                  </td>
      ...
      <div className="bg-white dark:bg-[#1C1C1F] border border-slate-200 dark:border-[#2e2e32] rounded-xl overflow-hidden shadow-sm">
        ...
        <div className="divide-y divide-slate-100 dark:divide-[#2e2e32]">
          {(rollup?.vendor_overlaps ?? []).length === 0 ? (
            ...
            (rollup?.vendor_overlaps ?? []).map((vendor) => (
              <div key={vendor.vendor_id} className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-50/80 dark:hover:bg-[#252529]/40 transition-colors">
                <div className="space-y-1.5 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-slate-900 dark:text-white text-sm">{vendor.vendor_name}</span>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 pt-1">
                    <span className="text-[11px] text-slate-500 dark:text-neutral-500">Active in:</span>
                    {vendor.entity_names.map((name, i) => (
                      <span key={i} className="px-2 py-0.5 rounded text-[10px] bg-slate-100 dark:bg-[#252529] border border-slate-200 dark:border-[#2e2e32] text-slate-700 dark:text-neutral-300">
                        {name}
                      </span>
                    ))}
                  </div>
      ...
      <div className="bg-white dark:bg-[#1C1C1F] border border-slate-200 dark:border-[#2e2e32] rounded-xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(rollup?.top_categories ?? []).map((cat, i) => (
            <div key={i} className="p-4 rounded-lg bg-slate-50 dark:bg-[#252529]/60 border border-slate-200 dark:border-[#2e2e32] flex items-center justify-between">
              <div>
                <div className="text-xs font-semibold text-slate-800 dark:text-neutral-200">{cat.category_name}</div>
