...
import { usePayments, useProcessPayment } from "@procurement/hooks";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-slate-200/80 shadow-xs">
          ...
          <span className="text-xs text-slate-500 mt-0.5 block">{kpis.completedCount} settled disbursements</span>
        </div>

        <div className="bg-sky-50/50 backdrop-blur-md p-4 rounded-2xl border border-sky-200/80 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-sky-700">Scheduled / Due</span>
          ...
          <span className="text-xs text-sky-600 mt-0.5 block">{kpis.scheduledCount} pending bank execution</span>
        </div>

        <div className="bg-amber-50/50 backdrop-blur-md p-4 rounded-2xl border border-amber-200/80 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-amber-700">TDS Deducted</span>
            <ShieldAlert className="h-4 w-4 text-amber-600" />
          </div>
          <p className="text-2xl font-bold text-amber-900 mt-2">
            ₹{kpis.totalTds.toLocaleString("en-IN", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
          </p>
          <span className="text-xs text-amber-700 mt-0.5 block">Statutory withholding (Sec 194C/J)</span>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-slate-200/80 shadow-xs">
          ...
          <p className="text-2xl font-bold text-slate-900 mt-2">{kpis.totalTransactions}</p>
          <span className="text-xs text-slate-500 mt-0.5 block">Total disbursement entries</span>
        </div>
      </div>
                    ...
                    <tr key={p.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-3 py-3.5">
                        <Link
                          ...
                          className="font-medium text-indigo-600 hover:text-indigo-800 flex items-center gap-1 group"
                        >
                      ...
                      {/* Vendor */}
                      <td className="px-3 py-3.5">
                        <div className="flex items-center gap-1.5 text-slate-800 font-medium max-w-[160px] truncate">
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-slate-200 space-y-5 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/80 space-y-2 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-900">{selectedPayment.invoice_number || "—"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Beneficiary Vendor:</span>
                <span className="font-semibold text-slate-900">{selectedPayment.vendor_name || "—"}</span>
              </div>
              <div className="border-t border-slate-200 my-2 pt-2 flex justify-between text-slate-600">
            ...
            <form onSubmit={handleProcessSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                />
              ...
              <div>
                <label className="font-semibold text-slate-700 block mb-1">ERP Disbursement Reference (Optional)</label>
                <input
                  type="text"
