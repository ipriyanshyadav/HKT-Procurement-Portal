...
export default function SupplierPaymentsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status?.toUpperCase()) {
      ...
      case "PROCESSING":
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Credited to Account</span>
          ...
          <span className="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5 block">{kpis.settledCount} settled remittances</span>
        </div>

        <div className="bg-sky-50/50 dark:bg-sky-950/30 backdrop-blur-md p-4 rounded-2xl border border-sky-200/80 dark:border-sky-800/60 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-sky-700 dark:text-sky-300">Scheduled Inflow</span>
          ...
          <span className="text-xs text-sky-600 dark:text-sky-400 mt-0.5 block">Approved & awaiting payout</span>
        </div>

        <div className="bg-amber-50/50 dark:bg-amber-950/30 backdrop-blur-md p-4 rounded-2xl border border-amber-200/80 dark:border-amber-800/60 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-amber-700 dark:text-amber-300">TDS Credit (2%)</span>
          ...
          <span className="text-xs text-amber-700 dark:text-amber-400 mt-0.5 block">Eligible for Form 26AS / TRACES</span>
        </div>
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
          ...
          <p className="text-2xl font-bold text-slate-900 dark:text-white mt-2">{kpis.totalCount}</p>
          <span className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 block">Total remittance records</span>
        </div>
      </div>

      {/* Statutory Info Banner */}
      <div className="bg-emerald-50/60 dark:bg-emerald-950/30 rounded-2xl p-4 border border-emerald-100 dark:border-emerald-900/50 flex items-start gap-3">
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 shadow-xs flex flex-col sm:flex-row gap-3 items-center justify-between">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <select
            ...
            className="px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-sm bg-white dark:bg-slate-800/90 text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500"
          >
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {(p.status === "COMPLETED" || p.status === "PAID") && (
                            <button
                              ...
                              className="inline-flex items-center gap-1 px-2.5 py-1.5 border border-emerald-200 dark:border-emerald-800/80 bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 text-emerald-800 dark:text-emerald-300 rounded-lg text-xs font-semibold shadow-xs transition-colors disabled:opacity-50"
                            >
