...
export default function SupplierDocumentsPage() {
  ...
  return (
    <div className="w-full space-y-6">
      {/* Header */}
      <div className="bg-white dark:bg-[#1C1C1F] rounded-2xl border border-slate-200 dark:border-white/15 p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Documents & Compliance</h1>
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/profile"
            className="px-4 py-2 bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/15 text-slate-700 dark:text-slate-200 text-sm font-medium rounded-lg transition-colors"
          >
            ← Back to Profile
          </Link>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/40 dark:to-indigo-950/40 border border-blue-200 dark:border-blue-800/60 rounded-2xl p-4 flex items-center gap-3 text-sm text-blue-900 dark:text-blue-200">
        <span className="text-xl">🛡️</span>
        <div>
          <p className="font-semibold text-blue-950 dark:text-blue-100">Automated Malware & Anti-Virus Protection Active</p>
      ...
      <div className="bg-white dark:bg-[#1C1C1F] rounded-2xl border border-slate-200 dark:border-white/15 p-6 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {COMPLIANCE_REQUIREMENTS.map((req) => (
            <div
              key={req.code}
              className="p-4 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#252529] hover:border-blue-400 dark:hover:border-blue-400/50 transition-all shadow-xs flex flex-col justify-between gap-3"
            >
              <div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-white/10 text-xs">
                <span className="text-[11px] font-mono text-slate-500 dark:text-slate-400">{req.allowedFormats}</span>
                {req.hasTemplate ? (
                  <button
                  ...
                  <span className="text-[11px] text-slate-400 dark:text-slate-500 italic">Issued by Authority</span>
                )}
              </div>
            </div>
          ))}
      ...
      {vendorLoading ? (
        <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400 bg-white dark:bg-[#1C1C1F] rounded-2xl border border-slate-200 dark:border-white/15">
