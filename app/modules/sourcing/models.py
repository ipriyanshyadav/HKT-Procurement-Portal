...
class Rfq(BaseModel):
    ...
    sourcing_type: Mapped[SourcingTypeEnum] = mapped_column(
        SOURCING_TYPE_PG, default=SourcingTypeEnum.GOODS, nullable=False
    )
    evaluation_type: Mapped[EvaluationTypeEnum] = mapped_column(
        EVALUATION_TYPE_PG, default=EvaluationTypeEnum.L1_PRICE_ONLY, nullable=False
    )
    procurement_type: Mapped[ProcurementTypeEnum] = mapped_column(
        PROCUREMENT_TYPE_PG, default=ProcurementTypeEnum.OPEX, nullable=False
    )
    ...
    payment_term_id: Mapped[UUID | None] = mapped_column(nullable=True)
    incoterm_id: Mapped[UUID | None] = mapped_column(nullable=True)
    delivery_location_id: Mapped[UUID | None] = mapped_column(nullable=True)
    # bid_open_at = when bids CAN be opened (the scheduled opening datetime)
    bid_open_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    # bid_close_at = submission deadline (bids must be received by this time)
    bid_close_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    technical_close_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    single_vendor_justification: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    bids_opened_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    bids_opened_by: Mapped[UUID | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    co_authorized_by: Mapped[UUID | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    # Dual-auth Step 1 fields
    bid_opening_initiated_by: Mapped[UUID | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    bid_opening_initiated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    source_pr_id: Mapped[UUID | None] = mapped_column(ForeignKey("requisitions.id"), nullable=True)
    bidding_mode: Mapped[str] = mapped_column(String(32), default="SEALED", nullable=False)
    auction_config: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    ...
    clarifications: Mapped[list[RfqClarification]] = relationship(
        "RfqClarification", back_populates="rfq", lazy="selectin"
    )
...
class RfqLot(BaseModel):
    ...
    rfq_id: Mapped[UUID] = mapped_column(ForeignKey("rfqs.id"), index=True, nullable=False)
    lot_number: Mapped[int] = mapped_column(Integer, nullable=False)
    title: Mapped[str] = mapped_column(String(200), nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    estimated_value: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=Decimal("0.0"), nullable=False)
    payment_term_override_id: Mapped[UUID | None] = mapped_column(nullable=True)
    incoterm_override_id: Mapped[UUID | None] = mapped_column(nullable=True)
...
class RfqParticipant(BaseModel):
    ...
    regretted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    regret_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class RfqClarification(BaseModel):
    ...
    asked_by_vendor_id: Mapped[UUID | None] = mapped_column(ForeignKey("vendors.id"), nullable=True)
    answered_by: Mapped[UUID | None] = mapped_column(ForeignKey("users.id"), nullable=True)
...
class RfqAmendment(BaseModel):
    ...
    previous_bid_close_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    new_bid_close_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
