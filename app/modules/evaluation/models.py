...
from app.db.enums import EVALUATION_TYPE_PG, EvaluationTypeEnum
...
class ComparativeStatement(BaseModel):
    ...
    l1_total_value: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True)
    savings_percentage: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), nullable=True)
...
class CsLineRanking(Base):
    ...
    rfq_line_id: Mapped[UUID | None] = mapped_column(ForeignKey("rfq_lines.id"), nullable=True)
    ...
    supplier_declared_rate: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), nullable=True)
    hsn_master_rate: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), nullable=True)
    tie_breaking_applied: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    tie_breaking_reason: Mapped[str | None] = mapped_column(String(200), nullable=True)
    lot_total_inr: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    technical_score: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), nullable=True)
    commercial_score: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), nullable=True)
    composite_score: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), nullable=True)
...
class Negotiation(BaseModel):
    ...
    rfq_id: Mapped[UUID] = mapped_column(ForeignKey("rfqs.id"), nullable=False)
    cs_id: Mapped[UUID | None] = mapped_column(ForeignKey("comparative_statements.id"), nullable=True)
    vendor_id: Mapped[UUID] = mapped_column(ForeignKey("vendors.id"), nullable=False)
    round_number: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    proposed_price: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    counter_price: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    original_price: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    negotiated_price: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    price_change_pct: Mapped[Decimal | None] = mapped_column(Numeric(7, 4), nullable=True)
    status: Mapped[str] = mapped_column(String(20), default="OPEN", nullable=False)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    negotiated_by: Mapped[UUID] = mapped_column(ForeignKey("users.id"), nullable=False)
    initiated_by: Mapped[UUID | None] = mapped_column(ForeignKey("users.id"), nullable=True)

    comparative_statement: Mapped[ComparativeStatement | None] = relationship("ComparativeStatement", back_populates="negotiations")
...
class AwardRecommendation(BaseModel):
    ...
    total_awarded_value: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True)
