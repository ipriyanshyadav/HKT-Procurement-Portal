...
from sqlalchemy import CHAR, Boolean, Date, DateTime, FetchedValue, ForeignKey, Integer, Numeric, String, Text
...
class Requisition(BaseModel):
    ...
    delivery_location_id: Mapped[UUID | None] = mapped_column(ForeignKey("delivery_locations.id"), nullable=True)
    erp_pr_number: Mapped[str | None] = mapped_column(String(30), nullable=True)
    erp_sync_status: Mapped[str] = mapped_column(String(20), default="NOT_SYNCED", nullable=False)
    merged_from: Mapped[list[UUID] | None] = mapped_column(ARRAY(PG_UUID(as_uuid=True)), nullable=True)
    split_into: Mapped[list[UUID] | None] = mapped_column(ARRAY(PG_UUID(as_uuid=True)), nullable=True)
    split_from: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), nullable=True)
...
class RequisitionLine(BaseModel):
    ...
    requisition_id: Mapped[UUID] = mapped_column(ForeignKey("requisitions.id"), index=True, nullable=False)
    ...
    estimated_total: Mapped[Decimal | None] = mapped_column(
        Numeric(18, 2), server_default=FetchedValue(), nullable=True
    )
    hsn_code: Mapped[str | None] = mapped_column(String(10), nullable=True)
    specifications: Mapped[Text | None] = mapped_column(Text, nullable=True)
    required_by_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    delivery_location_id: Mapped[UUID | None] = mapped_column(ForeignKey("delivery_locations.id"), nullable=True)
...
class UnmappedPrException(Base):
    ...
    status: Mapped[UnmappedPrStatusEnum] = mapped_column(
        UNMAPPED_PR_STATUS_PG, default=UnmappedPrStatusEnum.PENDING, nullable=False
    )
    ...
    proposed_mappings: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
    ...
    last_reprocessing_error: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class UnmappedPrMappingLog(Base):
    ...
    checked_by: Mapped[UUID | None] = mapped_column(ForeignKey("users.id"), nullable=True)
