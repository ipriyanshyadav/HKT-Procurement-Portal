...
class VendorRepository(BaseRepository[Vendor]):
    ...
    async def find_by_id(
        self, db: AsyncSession, vendor_id: UUID, org_id: UUID | None = None
    ) -> Vendor | None:
    ...
    async def find_by_ids(
        self, db: AsyncSession, vendor_ids: list[UUID], org_id: UUID | None = None
    ) -> list[Vendor]:
        if not vendor_ids:
            return []
    ...
    async def create_onboarding_application(
        ...
        app = VendorOnboardingApplication(
            org_id=org_id,
            vendor_id=vendor_id,
    ...
    async def get_onboarding_application(
        self, db: AsyncSession, app_id: UUID, org_id: UUID
    ) -> VendorOnboardingApplication | None:
        stmt = select(VendorOnboardingApplication).where(
            VendorOnboardingApplication.id == app_id,
            VendorOnboardingApplication.org_id == org_id,
            VendorOnboardingApplication.deleted_at.is_(None),
        )
        res = await db.execute(stmt)
        return res.scalar_one_or_none()
    ...
    ) -> list[tuple[VendorOnboardingApplication, Vendor]]:
        stmt = (
            select(VendorOnboardingApplication, Vendor)
            .join(Vendor, Vendor.id == VendorOnboardingApplication.vendor_id)
            .where(
                VendorOnboardingApplication.org_id == org_id,
                VendorOnboardingApplication.deleted_at.is_(None),
                Vendor.deleted_at.is_(None),
            )
            .order_by(desc(VendorOnboardingApplication.created_at))
            .offset(skip)
            .limit(limit)
        )
        res = await db.execute(stmt)
