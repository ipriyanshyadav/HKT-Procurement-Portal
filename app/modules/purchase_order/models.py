...
from app.db.enums import PO_STATUS_PG, PoStatusEnum
...
class PurchaseOrder(BaseModel):
    ...
    erp_po_number: Mapped[str | None] = mapped_column(String(30), nullable=True)
    erp_sync_status: Mapped[str] = mapped_column(String(20), default="NOT_SYNCED", nullable=False)
    po_pdf_document_id: Mapped[UUID | None] = mapped_column(ForeignKey("documents.id"), nullable=True)
    po_document_path: Mapped[str | None] = mapped_column(String(500), nullable=True)
    ...
    vendor_rejection_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    lines: Mapped[list[PoLine]] = relationship(
        "PoLine", back_populates="po", cascade="all, delete-orphan", order_by="PoLine.line_number"
    )
    amendments: Mapped[list[PoAmendment]] = relationship(
        "PoAmendment", back_populates="po", cascade="all, delete-orphan", order_by="PoAmendment.amendment_number"
    )
...
class PoLine(BaseModel):
    ...
    po_id: Mapped[UUID] = mapped_column(ForeignKey("purchase_orders.id"), index=True, nullable=False)
    ...
    total_price: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), server_default=FetchedValue(), nullable=True)
    awarded_unit_price: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
